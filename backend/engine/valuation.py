...
def get_primary_position(pos_string, player_name=None):
    ...
    # Standardize all Midfielders (CM, DM, AM, MF) to one 'MF' group for ease
    return 'MF'
...
def calculate_valuation(player_name):
    ...
    # Check to see if player exists in both databases
    ...
    conn.close()
    if peers_df.empty: peers_df = df_2526
    # Percentile calculation for each stat
    percentiles = {}
    for stat, higher_is_better in metrics.items():
        # Check if the stat exists in the data
        if stat not in peers_df.columns:
            continue
        # Clean the data (Force to numbers, handle NaNs)
        ...
        if pd.isna(p_val):
            p_val = 0
        # Step D: Scale correction (Fixes 0.89 vs 89.9 issue)
        ...
        # Apply Logic Flipped (e.g., for 'Turnovers' lower is better)
    ...
    # Goal Scorer scarcity premium since they're rare nowadays
